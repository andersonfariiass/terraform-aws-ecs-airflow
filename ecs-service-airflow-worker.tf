module "ecs_service_airflow_worker" {
  source   = "terraform-aws-modules/ecs/aws//modules/service"
  version  = "5.11.3"
  for_each = { for svc in local.workspace.service_worker : svc.name => svc }
  ...
  container_definitions = {
    (each.value.name) = {
      ...
      command = can(each.value.command) ? [for cmd in each.value.command : cmd] : []
